...
class System:
    ...
    async def update_firmware_version_info_to_cloud(self, h: Http):
        ...
        req = h.create_new_http_request(url=update_firmware_url, header=header, body_data=firmware_report_body_data)
        session = aiohttp.ClientSession()
        res = await h.post(session, req)
        try:
            data = await res.json()
            self.__logger.info(f"Update firmware info: {data}")
            print(f"Update firmware info: {data}")
        except: 
            self.__logger.debug(f"fail to send update firmware info")
        await session.close()